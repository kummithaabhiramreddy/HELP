...
async function handler(req, res) {
  ...
  if (req.method === 'POST' && (pathname === '/api/donors' || pathname === '/api/register')) {
    ...
    try {
      ...
      if (dup) {
        console.log(`♻️ Existing donor found (${dup.donorId}). Checking cooldown rules.`);

        // Check cooldown for Blood donors (both previous and new registration must be Blood/Both)
        if ((body.type === 'Blood' || body.type === 'Both') && (dup.type === 'Blood' || dup.type === 'Both')) {
          ...
          if (daysPassed < 90) {
            ...
            return sendJSON(res, 400, {
              error: `You have already registered for blood donation within the last 3 months. Next registration is allowed after ${daysLeft} day(s).`
            });
          }
        }

        // Merge registration types and details if there are changes
        let newType = dup.type;
        if (body.type === 'Both' || dup.type === 'Both') {
          newType = 'Both';
        } else if ((dup.type === 'Blood' && body.type === 'Organ') || (dup.type === 'Organ' && body.type === 'Blood')) {
          newType = 'Both';
        } else if (body.type) {
          newType = body.type;
        }

        let newBloodGroup = body.bloodgroup && body.bloodgroup !== 'N/A' && body.bloodgroup !== 'Not Applicable' ? body.bloodgroup : dup.bloodgroup;

        let mergedOrgans = dup.organs || [];
        if (Array.isArray(body.organs)) {
          mergedOrgans = Array.from(new Set([...mergedOrgans, ...body.organs]));
        }
        ...
        sendRegistrationEmail({ ...dup, ...body, type: newType, bloodgroup: newBloodGroup, organs: mergedOrgans, donated_count: newCount }).catch(e => console.error('Email update trigger failed:', e));
  ...
  /* ══════════════════════════════════════════════
     POST /api/emergency/intake — generic patient intake
  ══════════════════════════════════════════════ */
  if (req.method === 'POST' && pathname === '/api/emergency/intake') {
    let body;
    try {
      body = await readBody(req);
    } catch (err) {
    ...
    try {
      ...
      const referenceId = body.referenceId || 'GENERAL_SOS';
      
      await dbRepo.logEmergencyRequest({
        donorId: referenceId,
        requesterName: body.patientName || 'Unknown Patient',
        requestType: body.needType || 'Unknown',
        bloodGroup: body.bloodGroup || null,
        organType: (body.organs && body.organs.length > 0) ? body.organs.join(', ') : null,
        details: detailsJson
      });
      
      return sendJSON(res, 200, { success: true, referenceId: referenceId });
    } catch (err) {
  ...
  if (req.method === 'GET' && pathname.startsWith('/api/donor/')) {
    try {
        ...
        try {
          ...
          return str.split(',').map(s => {
            ...
            return {
              ...
              hospital: 'HELP Network Hospital',
              notes: trimmed
            };
          }).filter(Boolean);
        }
      ...
      const donorData = {
        ...donor,
        donated_detail: safeParse(donor.donated_detail),
        received_detail: safeParse(donor.received_detail),
  ...
  /* ══════════════════════════════════════════════
     GET /api/auth/google-check — check if email exists
  ══════════════════════════════════════════════ */
  if (req.method === 'GET' && pathname === '/api/auth/google-check') {
  ...
  /* ══════════════════════════════════════════════
     GET /api/stats/live — live splash screen counts
     Returns: total_users, blood_donors, organ_donors
  ══════════════════════════════════════════════ */
  if (req.method === 'GET' && pathname === '/api/stats/live') {
    try {
      const byType = await dbRepo.countByType();
      const userStats = await dbRepo.getStats();

      let blood = 0, organ = 0;
      for (const row of byType) {
        const t = (row.type || '').toLowerCase();
        if (t === 'blood') blood += Number(row.count);
        if (t === 'organ') organ += Number(row.count);
        if (t === 'both') { blood += Number(row.count); organ += Number(row.count); }
      }
      ...
      return sendJSON(res, 500, { error: 'Failed to fetch live stats' });
    }
  }
